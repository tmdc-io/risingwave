...
/**
 * Single message transform that modifies records coming from TimescaleDB. It operates on chunks and
 * aggregate's materialized hypertables.
 *
 * <ul>
 *   <li>chunk source info block and topic name is replaced with appropriate hypertable name
 *   <li>chunks belonging to hypertables acting as materialized aggregates have topic name and
 *       source info block updated with aggregate name
 *   <li>headers are added with original chunk schema/table name and for aggregates with original
 *       materialized hypertable schema/table name
 * </ul>
 *
...
public class TimescaleDb<R extends ConnectRecord<R>> implements Transformation<R>, Versioned {
    ...
    public static final String HEADER_HYPERTABLE_SCHEMA =
    ...
    public R apply(R record) {
        ...
        if (schemaName == null || tableName == null) {
            LOGGER.debug(
                    "Incoming record has an empty schema '{}' or table '{}' name",
                    schemaName,
                    tableName);
            return record;
        }
        ...
        if (!metadata.isTimescaleDbSchema(schemaName)) {
            LOGGER.trace(
                    "Record for schema '{}' is not intended for TimescaleDB processing",
                    schemaName);
            return record;
        }
        ...
        // If the data are coming from aggregate then we should use its metadata, otherwise
        // hypertable metadata are used
        if (aggregateId.isPresent()) {
            LOGGER.trace("Changing metadata for aggregate from '{}' to '{}'", chunkId, aggregateId);
        } else {
            LOGGER.trace(
                    "Changing metadata for hypertable from '{}' to '{}'", chunkId, aggregateId);
        }
    ...
    private Headers addHeaders(
            Headers headers, TableId chunkId, TableId hypertableId, Optional<TableId> aggregateId) {

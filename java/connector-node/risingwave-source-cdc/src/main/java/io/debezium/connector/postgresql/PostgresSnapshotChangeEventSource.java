...
public class PostgresSnapshotChangeEventSource
    ...
    public SnapshottingTask getSnapshottingTask(
        ...
        Map<DataCollectionId, String> snapshotSelectOverridesByTable =
                connectorConfig.getSnapshotSelectOverridesByTable();
        ...
        boolean snapshotData =
                snapshotterService
                        .getSnapshotter()
                        .shouldSnapshotData(offsetExists, snapshotInProgress);
        if (snapshotData) {
    ...
    protected SnapshotContext<PostgresPartition, PostgresOffsetContext> prepare(
            PostgresPartition partition, boolean onDemand) {
    ...
    protected void connectionCreated(
        ...
        if (snapshotterService.getSnapshotter().shouldStreamEventsStartingFromSnapshot()
    ...
    protected void lockTablesForSchemaSnapshot(
        ...
        final Set<String> capturedTablesNames =
                snapshotContext.capturedTables.stream()
                        .map(TableId::toDoubleQuotedString)
                        .collect(Collectors.toSet());

        List<String> tableLockStatements =
                capturedTablesNames.stream()
                        .map(
                                tableId ->
                                        snapshotterService
                                                .getSnapshotLock()
                                                .tableLockingStatement(lockTimeout, tableId))
        ...
        if (!tableLockStatements.isEmpty()) {
            ...
            // we're locking in ACCESS SHARE MODE to avoid concurrent schema changes while we're
            // taking the snapshot
            // this does not prevent writes to the table, but prevents changes to the table's
            // schema....
            // DBZ-298 Quoting name in case it has been quoted originally; it doesn't do harm if it
            // hasn't been quoted
            tableLockStatements.forEach(
                    tableStatement -> statements.append(tableStatement).append(lineSeparator));
    ...
    protected void determineSnapshotOffset(
        ...
        if (offset == null) {
            if (previousOffset != null
                ...
                offset =
                        PostgresOffsetContext.initialContext(
                                connectorConfig,
                                jdbcConnection,
                                getClock(),
    ...
    protected void updateOffsetForPreSnapshotCatchUpStreaming(PostgresOffsetContext offset)
            throws SQLException {
        updateOffsetForSnapshot(offset);
    ...
    private Lsn getTransactionStartLsn() throws SQLException {
        ...
        } else if (!snapshotterService.getSnapshotter().shouldStreamEventsStartingFromSnapshot()
                && startingSlotInfo != null) {
    ...
    protected void readTableStructure(
        ...
        for (String schema : schemas) {
            ...
            Tables.TableFilter tableFilter =
                    snapshottingTask.isOnDemand()
                            ? Tables.TableFilter.fromPredicate(
                                    snapshotContext.capturedTables::contains)
    ...
    protected Optional<String> getSnapshotSelect(
        ...
        return snapshotterService
                .getSnapshotQuery()
                .snapshotQuery(tableId.toDoubleQuotedString(), columns);
    }
    ...
    protected void setSnapshotTransactionIsolationLevel(boolean isOnDemand) throws SQLException {
        ...
        String transactionStatement =
                snapshotTransactionIsolationLevelStatement(slotCreatedInfo, isOnDemand);
    ...
    private String snapshotTransactionIsolationLevelStatement(
            SlotCreationResult newSlotInfo, boolean isOnDemand) {
    ...
    private static class PostgresSnapshotContext
            extends RelationalSnapshotContext<PostgresPartition, PostgresOffsetContext> {
    ...
    @Override
    protected PostgresOffsetContext copyOffset(
            RelationalSnapshotContext<PostgresPartition, PostgresOffsetContext> snapshotContext) {

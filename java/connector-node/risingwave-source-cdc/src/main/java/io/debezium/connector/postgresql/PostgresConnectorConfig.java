...
public class PostgresConnectorConfig extends RelationalDatabaseConnectorConfig {
    ...
    public enum HStoreHandlingMode implements EnumeratedValue {
         ...
         * @return the matching option or null if the match is not found and non-null default is
         *     invalid
         */
    ...
    public enum SnapshotMode implements EnumeratedValue {
        ...
        /**
         * Allows control over snapshots by setting connectors properties prefixed with
         * 'snapshot.mode.configuration.based'.
         */
    ...
    public enum SecureConnectionMode implements EnumeratedValue {
        ...
        /**
         * Establish an unencrypted connection first. Establish a secure connection next if an
         * unencrypted connection cannot be established
         *
         * <p>see the {@code sslmode} Postgres JDBC driver option
         */
        ALLOW("allow"),
        ...
        /**
         * Establish a secure connection first. Establish an unencrypted connection next if a secure
    ...
    public enum SchemaRefreshMode implements EnumeratedValue {
        /**
         * Refresh the in-memory schema cache whenever there is a discrepancy between it and the
         * schema derived from the incoming message.
         */
         ...
         * However, it is possible for the in-memory schema to become outdated if TOASTable columns
    ...
    public enum SnapshotLockingMode implements EnumeratedValue {
        /**
         * This mode will lock in ACCESS SHARE MODE to avoid concurrent schema changes during the
         * snapshot, and this does not prevent writes to the table, but prevents changes to the
         * table's schema.
         */
        SHARED("shared"),
    ...
    public static final Field DROP_SLOT_ON_STOP =
            Field.create("slot.drop.on.stop")
                    ...
                    .withGroup(
                            Field.createGroupEntry(Field.Group.CONNECTION_ADVANCED_REPLICATION, 3))
                    .withDefault(false)
                    .withImportance(Importance.MEDIUM)
                    .withDescription(
    ...
    public static final Field PUBLICATION_NAME =
            Field.create("publication.name")
                    ...
                    .withType(Type.STRING)
                    .withGroup(
                            Field.createGroupEntry(Field.Group.CONNECTION_ADVANCED_REPLICATION, 8))
                    .withWidth(Width.MEDIUM)
                    .withImportance(Importance.MEDIUM)
    ...
    public enum AutoCreateMode implements EnumeratedValue {
        /**
         * No Publication will be created, it's expected the user has already created the
         * publication.
         */
        DISABLED("disabled"),
    ...
    public static final Field PUBLICATION_AUTOCREATE_MODE =
            Field.create("publication.autocreate.mode")
                    ...
                    .withGroup(
                            Field.createGroupEntry(Field.Group.CONNECTION_ADVANCED_REPLICATION, 9))
    ...
    public static final Field REPLICA_IDENTITY_AUTOSET_VALUES =
            Field.create("replica.identity.autoset.values")
                    .withDisplayName("Replica Identity Auto Set Values")
                    .withType(Type.STRING)
                    .withGroup(
                            Field.createGroupEntry(Field.Group.CONNECTION_ADVANCED_REPLICATION, 10))
    ...
    public static final Field STREAM_PARAMS =
            Field.create("slot.stream.params")
                    ...
                    .withType(Type.STRING)
                    .withGroup(
                            Field.createGroupEntry(Field.Group.CONNECTION_ADVANCED_REPLICATION, 2))
                    .withWidth(Width.LONG)
                    .withImportance(Importance.LOW)
                    .withDescription(
    ...
    public static final Field RETRY_DELAY_MS =
            Field.create("slot.retry.delay.ms")
                    ...
                    .withType(Type.LONG)
                    .withGroup(
                            Field.createGroupEntry(Field.Group.CONNECTION_ADVANCED_REPLICATION, 5))
                    .withImportance(Importance.LOW)
    ...
    public static final Field SNAPSHOT_LOCKING_MODE =
            Field.create("snapshot.locking.mode")
                    ...
                    .withDescription(
                            "Controls how the connector holds locks on tables while performing the schema snapshot. The 'shared' "
    ...
    /**
     * A comma-separated list of regular expressions that match the prefix of logical decoding
     * messages to be excluded from monitoring. Must not be used with {@link
    ...
    public static final Field LOGICAL_DECODING_MESSAGE_PREFIX_EXCLUDE_LIST =
            Field.create("message.prefix.exclude.list")
                    ...
                    .withValidation(
                            Field::isListOfRegex,
                            PostgresConnectorConfig::validateLogicalDecodingMessageExcludeList)
                    .withDescription(
    ...
    /**
     * A comma-separated list of regular expressions that match the prefix of logical decoding
     * messages to be monitored. Must not be used with {@link
    ...
    public static final Field STATUS_UPDATE_INTERVAL_MS =
            Field.create("status.update.interval.ms")
                    ...
                    .withGroup(
                            Field.createGroupEntry(Field.Group.CONNECTION_ADVANCED_REPLICATION, 6))
                    .withDefault(10_000)
                    .withWidth(Width.SHORT)
                    .withImportance(Importance.MEDIUM)
                    .withDescription(
    ...
    public static final Field XMIN_FETCH_INTERVAL =
            Field.create("xmin.fetch.interval.ms")
                    ...
                    .withType(Type.LONG)
                    .withGroup(
                            Field.createGroupEntry(Field.Group.CONNECTION_ADVANCED_REPLICATION, 7))
                    .withWidth(Width.SHORT)
                    .withDefault(0L)
                    .withImportance(Importance.MEDIUM)
                    .withDescription(
    ...
    public static final Field SHOULD_FLUSH_LSN_IN_SOURCE_DB =
            Field.create("flush.lsn.source")
                    .withDisplayName(
                            "Boolean to determine if Debezium should flush LSN in the source database")
                    .withType(Type.BOOLEAN)
    ...
    public PostgresConnectorConfig(Configuration config) {
        ...
        String hstoreHandlingModeStr =
                config.getString(PostgresConnectorConfig.HSTORE_HANDLING_MODE);
        ...
        this.replicaIdentityMapper =
                (replicaIdentityMapping != null)
                        ? new ReplicaIdentityMapper(replicaIdentityMapping)
                        : null;
        this.snapshotMode =
                SnapshotMode.parse(
    ...
    private static int validateFlushLsnSource(
            Configuration config, Field field, Field.ValidationOutput problems) {
        if (config.getString(PostgresConnectorConfig.SHOULD_FLUSH_LSN_IN_SOURCE_DB, "true")
                .equalsIgnoreCase("false")) {
            LOGGER.warn(
                    "Property '"
                            + PostgresConnectorConfig.SHOULD_FLUSH_LSN_IN_SOURCE_DB.name()
    ...
    protected static int validateReplicaAutoSetField(
            Configuration config, Field field, Field.ValidationOutput problems) {
        String replica_autoset_values =
                config.getString(PostgresConnectorConfig.REPLICA_IDENTITY_AUTOSET_VALUES);
        int problemCount = 0;

        if (replica_autoset_values != null) {
            if (replica_autoset_values.isEmpty()) {
                problems.accept(
                        PostgresConnectorConfig.REPLICA_IDENTITY_AUTOSET_VALUES,
                        "",
                        "Must not be empty");
            }

            for (String substring :
                    ReplicaIdentityMapper.PATTERN_SPLIT.split(replica_autoset_values)) {
                if (!ReplicaIdentityMapper.REPLICA_AUTO_SET_PATTERN.asPredicate().test(substring)) {
                    problems.accept(
                            PostgresConnectorConfig.REPLICA_IDENTITY_AUTOSET_VALUES,
                            substring,
                            substring
                                    + " has an invalid format (expecting '"
                                    + ReplicaIdentityMapper.REPLICA_AUTO_SET_PATTERN.pattern()
                                    + "')");
                    problemCount++;
                }
            }
    ...
    private static class SystemTablesPredicate implements TableFilter {
        ...
        // these are tables that may be placed in the user's schema but are system tables. This
        // typically includes modules
        ...
        public boolean isIncluded(TableId t) {
            return t.schema() != null
                    && !SYSTEM_SCHEMAS.contains(t.schema().toLowerCase())
                    && t.table() != null

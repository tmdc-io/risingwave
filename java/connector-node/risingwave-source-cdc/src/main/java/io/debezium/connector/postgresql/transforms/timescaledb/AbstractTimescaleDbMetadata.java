...
public abstract class AbstractTimescaleDbMetadata implements TimescaleDbMetadata {
    ...
    protected AbstractTimescaleDbMetadata(Configuration config) {
        timescaleDbSchemas =
                new HashSet<>(config.getList(TimescaleDbConfigDefinition.SCHEMA_LIST_NAMES_FIELD));
        if (timescaleDbSchemas.isEmpty()) {
            timescaleDbSchemas.add(
                    TimescaleDbConfigDefinition.SCHEMA_LIST_NAMES_FIELD.defaultValueAsString());
        }
    }
